...
DXE_MODULE_INFOS DxeModuleInfos = {0};
SMM_MODULE_INFOS SmmModuleInfos = {0};
//
...
UINT8 EFIAPI PCD_PROTOCOL_GET8_FUZZ(
  IN UINTN             TokenNumber
  )
{
  LIBAFL_QEMU_SMM_GET_PCD(1, (UINTN)&DummyPCD);
  DEBUG((DEBUG_INFO,"get pcd fuzz value %lx\n",DummyPCD));
  return (UINT8)DummyPCD;
}
UINT16 EFIAPI PCD_PROTOCOL_GET16_FUZZ(
  IN UINTN             TokenNumber
  )
{
  LIBAFL_QEMU_SMM_GET_PCD(2, (UINTN)&DummyPCD);
  DEBUG((DEBUG_INFO,"get pcd fuzz value %lx\n",DummyPCD));
  return (UINT16)DummyPCD;
}
UINT32 EFIAPI PCD_PROTOCOL_GET32_FUZZ(
  IN UINTN             TokenNumber
  )
{
  LIBAFL_QEMU_SMM_GET_PCD(4, (UINTN)&DummyPCD);
  DEBUG((DEBUG_INFO,"get pcd fuzz value %lx\n",DummyPCD));
  return (UINT32)DummyPCD;
}
UINT64 EFIAPI PCD_PROTOCOL_GET64_FUZZ(
  IN UINTN             TokenNumber
  )
{
  LIBAFL_QEMU_SMM_GET_PCD(8, (UINTN)&DummyPCD);
  DEBUG((DEBUG_INFO,"get pcd fuzz value %lx\n",DummyPCD));
  return (UINT64)DummyPCD; 
}
VOID * EFIAPI PCD_PROTOCOL_GET_POINTER_FUZZ(
  IN UINTN             TokenNumber
  )
{
  return (VOID*)0xb000000000000000;
}
BOOLEAN EFIAPI PCD_PROTOCOL_GET_BOOLEAN_FUZZ(
  IN UINTN             TokenNumber
  )
{
  LIBAFL_QEMU_SMM_GET_PCD(1, (UINTN)&DummyPCD);
  DEBUG((DEBUG_INFO,"get pcd fuzz value %lx\n",DummyPCD));
  return (BOOLEAN)DummyPCD;
}
UINTN EFIAPI PCD_PROTOCOL_GET_SIZE_FUZZ(
  IN UINTN             TokenNumber
  )
{
  return 8;
}
UINT8 EFIAPI PCD_PROTOCOL_GET_EX_8_FUZZ(
...
UINT16 EFIAPI PCD_PROTOCOL_GET_EX_16_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
UINT32 EFIAPI PCD_PROTOCOL_GET_EX_32_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
UINT64 EFIAPI PCD_PROTOCOL_GET_EX_64_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
VOID *EFIAPI PCD_PROTOCOL_GET_EX_POINTER_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
BOOLEAN EFIAPI PCD_PROTOCOL_GET_EX_BOOLEAN_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
UINTN EFIAPI PCD_PROTOCOL_GET_EX_SIZE_FUZZ(
  IN CONST EFI_GUID    *Guid,
  IN UINTN             TokenNumber
  )
{
...
EFI_STATUS EFIAPI DUMMY_PCD_FUZZ()
{
  return EFI_SUCCESS;
}
PCD_PROTOCOL  mPcdInstanceFuzz = {
  PCD_PROTOCOL_SET_SKU_FUZZ,
  PCD_PROTOCOL_GET8_FUZZ,
  PCD_PROTOCOL_GET16_FUZZ,
  PCD_PROTOCOL_GET32_FUZZ,
  PCD_PROTOCOL_GET64_FUZZ,
  PCD_PROTOCOL_GET_POINTER_FUZZ,
  PCD_PROTOCOL_GET_BOOLEAN_FUZZ,
  PCD_PROTOCOL_GET_SIZE_FUZZ,
  ...
  (PCD_PROTOCOL_SET8)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET16)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET32)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET64)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_POINTER)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_BOOLEAN)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_8)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_16)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_32)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_64)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_POINTER)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_SET_EX_BOOLEAN)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_CALLBACK_ONSET)DUMMY_PCD_FUZZ,
  (PCD_PROTOCOL_CANCEL_CALLBACK)DUMMY_PCD_FUZZ,
...
EFI_STATUS EFIAPI EFI_INSTALL_PROTOCOL_INTERFACE_FUZZ(
  ...
  if (IsCallFromFuzzModule ((UINT64)__builtin_return_address(0)) && CompareGuid(Protocol, &gEfiDriverBindingProtocolGuid))
    return EFI_SUCCESS;
  if (IsCallFromFuzzModule ((UINT64)__builtin_return_address(0))) {
    VOID *T;
...
BOOLEAN IsCallFromFuzzModule(UINT64 RetAddr) {
  if (SmmModuleInfos.data == NULL)
    return FALSE;
  for (UINTN i = 0; i < SmmModuleInfos.data->NumModules; i++)
  {
    UINT64 Start = (UINT64)SmmModuleInfos.data->info[i].ImageBase;
    UINT64 End = Start + (UINT64)SmmModuleInfos.data->info[i].ImageSize;
    if (RetAddr >= Start && RetAddr < End && !SmmModuleInfos.data->info[i].IsOvmfSmmModule)
    {
      return TRUE;
    }
  }
...
VOID InstallSmmFuzzProtocol() {
  ...
  ZeroMem (&DxeModuleInfos, sizeof(DxeModuleInfos));
  DxeModuleInfos.DxeModules[DxeModuleInfos.NumDxeModules].StartAddress = (UINTN)gDxeCoreLoadedImage->ImageBase;
  DxeModuleInfos.DxeModules[DxeModuleInfos.NumDxeModules].Size = gDxeCoreLoadedImage->ImageSize;
  CopyGuid(&DxeModuleInfos.DxeModules[DxeModuleInfos.NumDxeModules].Guid, gDxeCoreFileName);
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &Handle,
                  &gSmmFuzzDxeModuleInfoProtocolGuid,
                  &DxeModuleInfos,
                  NULL
                  );
  ASSERT_EFI_ERROR (Status);
